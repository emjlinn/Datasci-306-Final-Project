...
```{r}
#Is there a relationship between cigarette use prevalence, year, and mental health status?
smoking_all |> filter(disparity_category=="Mental Health") |> group_by(focus_group, Year) |>
  summarize(avg_prevalence = mean(focus_prev_pct)) |>
  ggplot(aes(x=Year, y=avg_prevalence, color=focus_group)) + 
  geom_line()
